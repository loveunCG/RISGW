...
function checkSessionTable() {
	knex.select('*').from('ci_sessions').then(function (rows) {
		for (var key in rows) {
...
async function updateMpps(params) {
	...
	knex.select('*').from('tbl_mpps').where(checkQuery).then((rows) => {
...
function updateBookingStatus(params) {
	...
	knex('tbl_check_list').where(params).select().then(getbkId => {
		...
		knex('tbl_check_list').where({
			chc_booking_id: booking_id
		}).select('checkup_status').then(checklistbybkid => {
			var isCheckup = 0;
			...
			if (isCheckup == checklistbybkid.length) {
				knex('tbl_patient_booking').where({
...
io.on('connection', function (socket) {
	...
	socket.on('connectThisRoom', data => {
		...
		knex('tbl_device').where({
			id: data.device_id
		}).update(updateData).then(data => {
			console.log(data);
		});
